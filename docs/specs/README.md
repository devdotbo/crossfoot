...
| `09-derived-targets.md` | Exact recomputation beyond svZCHF: Ethena sUSDe (five state reads, reward posts attributed), the Sky family (rpow, SPBEAM versus spell path), Ondo USDY, Frax sfrxUSD, Maple syrupUSDC and syrupUSDT (loan manager accounting) | outcome 1, added 2026-09-02 |
...
| `10-guard-wrapper.md` | `CrossfootGuard` and `CrossfootAttestations`: an AggregatorV3-compatible facade enforcing a consumer-owned posting policy on chain, with the Foundry prototype under `contracts/guard/` (Tectonic, mRE7 round 36, Sky and Venus replays) | prevention layer, prototype, added 2026-09-03; not one of the five |
| `11-guardian-agent.md` | Guardian agent: freezes a market within one block of a REVIEW through a protocol-granted role, deterministic rules, freeze only, audit trail as an actions bundle | design, added 2026-09-03; not one of the five |
| `12-accounting-invariants.md` | Compound v2 fork accounting check: cash identity, cash by transfer, exchange-rate replay; protocol-side, not a feed check | design, added 2026-09-03; not one of the five |
